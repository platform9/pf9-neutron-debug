...
class SNATInfo:
    ...
    def init_local_snat_process(self):
        ...
        local_listen_dict['network_label'] = self.get_network_label(self.external_network_id, self.neutron)
	    local_listen_dict['network_type'] = self.get_network_type(self.external_network_id, self.neutron)
        local_listen_dict['nic_filter'] = "icmp and  ((src %s and dst %s) or (src %s and dst %s)) " % (self.snat_external_ip, local_listen_dict['dest_ip_address'], local_listen_dict['dest_ip_address'], self.snat_external_ip)
    ...
    def init_remote_snat_process(self):
        ...
        local_listen_dict['host_id'] = self.source_host_id
        local_listen_dict['port_id'] = self.source_port_dict['id']
        local_listen_dict['network_label'] = self.get_network_label(self.tenant_network_id, self.neutron)
	    local_listen_dict['network_type'] = self.get_network_type(self.tenant_network_id, self.neutron)
        local_listen_dict['nic_filter'] = "icmp and  ((src %s and dst %s) or (src %s and dst %s)) " % (self.fixed_ip_address, local_listen_dict['dest_ip_address'], local_listen_dict['dest_ip_address'], self.fixed_ip_address)
        ...
        remote_listen_dict['network_label'] = self.get_network_label(self.external_network_id, self.neutron)
	    remote_listen_dict['network_type'] = self.get_network_type(self.external_network_id, self.neutron)
        remote_listen_dict['network_label_input'] = self.get_network_label(self.tenant_network_id, self.neutron)
	    remote_listen_dict['network_type_input'] = self.get_network_type(self.tenant_network_id, self.neutron)
        remote_listen_dict['nic_filter'] = "icmp and  ((src %s and dst %s) or (src %s and dst %s)) " % (self.snat_external_ip, remote_listen_dict['dest_ip_address'], remote_listen_dict['dest_ip_address'], self.snat_external_ip)
        ...
        if remote_listen_dict['network_type_input'] == "vxlan":
        ...
        remote_listen_dict['bridge_name'] = discovery.get_bridge_name(remote_listen_dict['network_label'], self.source_host_id, self.neutron)
        remote_listen_dict['bridge_name_input'] = discovery.get_bridge_name(remote_listen_dict['network_label_input'], self.source_host_id, self.neutron)
